...
Gsb-testgen
Automatic gsb test generation and replay.

Usage:
  gsb-testgen --replay <file>
  gsb-testgen --gsb <string>
  gsb-testgen --time <num> [options]
  gsb-testgen (-h | --help)
  gsb-testgen [options]
...
Options:
  ...
  --gsb FILE     The gsb executable to run.
...
impl Params {
	...
	// get the command to run gsb.
	fn gsb_command(&self) -> Command {
		let mut c = Command::new(self.args.flag_gsb.clone().unwrap_or("gsb".into()));
